...
export function WeekView({ selectedDate, tasks, onDateClick, onSlotClick, onTaskClick }: WeekViewProps) {
  ...
  return (
    <div className="flex h-full flex-col overflow-auto rounded-xl border border-[#dbe5f1] bg-white/88 shadow-sm">
      <div className="sticky top-0 z-10 grid grid-cols-[60px_repeat(7,1fr)] border-b border-[#dbe5f1] bg-white/92">
        <div className="p-2" />
        {days.map((d, i) => (
          <div key={i} className="cursor-pointer border-l border-slate-200 py-2 text-center hover:bg-blue-50/60"
            ...
            <div className={`mx-auto flex h-7 w-7 items-center justify-center rounded-full text-sm font-medium
              ${d.isSame(dayjs(), 'day') ? 'bg-blue-600 text-white' : 'text-slate-700'}`}>
              {d.date()}
            </div>
          </div>
        ))}
        ...
        {HOURS.map((hour) => (
          <div key={hour} className="contents">
            ...
            {days.map((d, di) => {
              ...
              return (
                <div key={di} className="relative h-12 border-l border-t border-slate-100 p-0.5 hover:bg-blue-50/40" onClick={() => onSlotClick(d.hour(hour).minute(0).second(0))}>
                  {slotTasks.map((t) => (
                    <button
                      key={t.id}
                      className="mb-0.5 block w-full truncate rounded bg-blue-50 px-1 text-left text-[10px] text-blue-700"
                      onClick={(event) => {
